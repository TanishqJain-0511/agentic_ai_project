...
def compute_financial_health(db: Session, user_id: int):
    profile = get_financial_profile_by_user_id(db, user_id)
    if not profile:
        return None
    ...
    net_worth = (profile.total_cash_savings + profile.total_existing_investments - profile.total_debt)

    savings_rate = (monthly_surplus / monthly_income * 100) if monthly_income > 0 else 0.0

    debt_to_income_ratio = (profile.total_debt / profile.annual_income * 100) if profile.annual_income > 0 else 0.0
...
def _dti_status(debt_to_income_ratio: float) -> str:
